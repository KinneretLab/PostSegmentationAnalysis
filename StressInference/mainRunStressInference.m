...
cellDir = [mainDir,'\Cells\Seg_for_training'];
segDir = [mainDir,'\Cells\Seg_for_training'];
...
for i=1:length(sortedFileNames)
    ...
    frameDir = [cellDir,'\',sortedFileNames{i}(1:(find(sortedFileNames{i}=='.')-1))];
    cd(frameDir);
...
% Remove all fourfold vertices, recursively if there are z>4 %YONIT:
