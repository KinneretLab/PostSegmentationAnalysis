...
for n=1:length(mainDirList)
   %% 1. Initialize parameters for each movie
    ...
    [fullCellDataRaw,fullVertexData] = extractCellData(segDir,maskDir,frameList{n});
    ...
    fullCellData = cells3DCorrection(mainDir,fullCellDataRaw,calibrationXY, calibrationZ,umCurvWindow,cellHMnum);
...
%     cd(cellDir); save('fullCellDataMod','fullCellDataMod');
    ...
    if exist([cellDir,'fullCellData.mat'], 'file') == 2
        ...
        fullCellDataOld = load('fullCellData'); fullCellDataOld = fullCellDataOld.fullCellData ;
